...
def _read_orders(db_path: str, retries: int = 8, sleep_s: float = 0.5) -> pd.DataFrame:
    """
    Robust reader for sqlite on network mounts (CIFS/NFS).
    Uses URI with nolock + immutable flags to bypass locking issues.
    Falls back to copy-to-tmp if URI mode fails.
    """
    import urllib.parse
    import tempfile
    import shutil
    ...
    # Method 1: URI with nolock (works on most network mounts)
    for i in range(retries):
        try:
            uri = "file:" + urllib.parse.quote(db_path) + "?mode=ro&nolock=1&immutable=1"
    ...
    # Method 2: Copy to local /tmp and read from there
    try:
        with tempfile.NamedTemporaryFile(suffix=".sqlite", delete=False) as tmp:
            tmp_path = tmp.name
        shutil.copy2(db_path, tmp_path)
        con = sqlite3.connect(tmp_path, timeout=5.0)
        df = pd.read_sql_query("SELECT * FROM orders", con)
        con.close()
        os.unlink(tmp_path)
        return df
